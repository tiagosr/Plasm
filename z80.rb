...
class Z80 < Assembler
  ...
  INSTRUCTION_TYPES = {
    none:  -> asm, opcode, tokens { asm.db opcode },
    cb:    -> asm, opcode, tokens { asm.db 0xcb, opcode },
    dd:    -> asm, opcode, tokens { asm.db 0xdd, opcode },
    ed:    -> asm, opcode, tokens { asm.db 0xed, opcode },
    fd:    -> asm, opcode, tokens { asm.db 0xfd, opcode },
    dds:   -> asm, opcode, tokens { asm.db 0xdd, opcode, tokens[0].if_within(-128,127).b },
    fds:   -> asm, opcode, tokens { asm.db 0xfd, opcode, tokens[0].if_within(-128,127).b },
    imm:   -> asm, opcode, tokens { asm.db opcode, tokens[0].if_within(-128,255).b },
    addr:  -> asm, opcode, tokens { asm.db opcode, tokens[0].if_within(-32768, 65535).lo_byte, tokens[0].hi_byte },
    rel:   -> asm, opcode, tokens { asm.db opcode, tokens[0].relative_to(asm.here).if_within(-128,127).b },
    bit:   -> asm, opcode, tokens { asm.db 0xcb, opcode + (0x08*tokens[0].if_within(0, 7)) },
    ddbit: -> asm, opcode, tokens { asm.db 0xdd, 0xcb, opcode + (0x08*tokens[0].if_within(0, 7)) },
    fdbit: -> asm, opcode, tokens { asm.db 0xfd, 0xcb, opcode + (0x08*tokens[0].if_within(0, 7)) },
    ddscb: -> asm, opcode, tokens { asm.db 0xdd, 0xcb, opcode, tokens[0].if_within(-128,127).b },
    fdscb: -> asm, opcode, tokens { asm.db 0xfd, 0xcb, opcode, tokens[0].if_within(-128,127).b },
    ldresix: -> asm, opcode, tokens { asm.db 0xdd, 0xcb, opcode + (0x08*tokens[0].if_within(0, 7)), tokens[1].if_within(-128,127).b },
    ldresiy: -> asm, opcode, tokens { asm.db 0xfd, 0xcb, opcode + (0x08*tokens[0].if_within(0, 7)), tokens[1].if_within(-128,127).b },
  }

  INSTRUCTION_WILDCARDS = {
    "s" => -> p { p.match("[-+]").as(:sign) >> p.spaces? >> p.expression.as(:expression) },
    "n" => -> p { p.expression.as(:imm) },
    "*" => -> p { p.match("[0-7]").as(:bit) },
    "?" => -> p { p.expression.as(:addr) },
    "@" => -> p { p.expression.as(:rel) },
  }
  class Z80InstructionParser < Assembler::AssemblerParser
      def self.make_z80_instructions (insn_table, type_table, wildcards)
        instruction_transformer = Parslet::Transformer.new do
            rule([simple(:insn), simple(:opcode), simple(:type)]) {
                insn_parser = OpcodeParser.new.apply(insn, :parser => self, :wildcards => INSTRUCTION_WILDCARDS)
            }
        end
      end
  end
